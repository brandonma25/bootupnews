...
## Fix
- Exact change: added a request id, safe structured stage diagnostics, sanitized failure fields, and explicit success/failure response metadata to the existing cron route and runner. Follow-up commits lazy-load the cron runner dependencies after route auth/env diagnostics, bypass auth-session proxy refresh for `/api/cron/*`, emit a single final cron diagnostic log, and suppress informational RSS boot console breadcrumbs so Vercel's request log view captures the failing stage instead of only the first progress line.
