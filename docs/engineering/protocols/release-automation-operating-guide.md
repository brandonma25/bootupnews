...
- The performance gate hard-fails homepage `/` when LCP or network idle exceeds `3000ms`; `2000ms` remains the visible-content optimization target.
- The performance report includes FCP, load event timing, network idle timing, decompressed homepage HTML size, script bytes, and route request count.
