...
### 2026-06-08 — classifyEventType correctness (the type the core gate reads)

The recalibration lift is only as good as the event TYPE the gate reads. `classifyEventType` (`src/lib/signal-filtering.ts`) was failing both ways: typing opinion/gossip/maneuvering as core types (importance then lifted them into the slate) and typing genuine legislation/security as `generic_commentary` (blocked regardless of importance). Fixed with tightened, framing-aware discriminators — opinion/think-piece/profile → `opinion_only`; process/maneuvering/blame-shifting → `generic_commentary`; legislative passage → `policy_regulation`; breach/hack → `cybersecurity_enforcement`; market-move → `macro_market_move`. No change to `event_importance`, the ≥52 gate, the `ranked.score` floor, blend weights, the evergreen filter, or `inferEventType`. Validated deterministically on the 06-08 fixture (3 false-positives → non-core, Meta-hack → cyber, 0 remaining opinion/gossip in core) and the live dry-run (0 evergreen leaks; #312/#313 lifts intact). See `docs/engineering/bug-fixes/2026-06-08-classify-event-type-false-positives-and-negatives.md` (Related PRD: PRD-38).
