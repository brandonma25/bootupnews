...
- 2026-06-08 — Re-ship of the evergreen fix bundled with a persist-floor softening (Track 2). The 2026-06-07 entry above shipped as #307 but was **reverted** (#308) after on-demand prod validation returned `Briefing failed: RSS=fail`, 0 rows: on a Fed-dominated day the P7 filter removed enough evergreens to drop the slate below the pre-existing ≥5 hard floor in `persistSignalPostCandidates`, which then refused the whole snapshot → empty slate. This re-ship re-applies #307 on top of two changes that make it safe — full write-up in [`docs/engineering/bug-fixes/evergreen-restaging-with-floor-softening-2026-06-08.md`](../../engineering/bug-fixes/evergreen-restaging-with-floor-softening-2026-06-08.md):
  - **Softened the persist floor**: `persistSignalPostCandidates` no longer hard-fails on a thin slate (1–4 source-ready) — it logs a degraded `warn` and persists, matching `runDailyNewsCron`'s 1–6 degraded behaviour (only 0 hard-fails). A thin CLEAN slate now persists instead of an empty one.
  - **Title-pattern audit**: the broad `^how (are|is|…)` / `^why (is|are|…)` prefixes are tightened — `^how (are|is|do|does|did|the)` now requires an explainer verb to follow, and `^why` is narrowed to `exclude|do|does` — so real news framed as a question is not suppressed. The feed signal (`EVERGREEN_PRONE_FEEDS`) remains the primary catch for the known Fed-blog evergreens.
