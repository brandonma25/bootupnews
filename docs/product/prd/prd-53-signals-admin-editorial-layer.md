...
- 2026-06-08 — Surfacing pool widening (PRD-53 remediation; initiative PR 1 of 3). Surfacing had collapsed to an effective hard top-N≈6 cap feeding a 5-slot public set, so the machine curated the slate instead of the editor — contrary to this PRD's editorial-authority promise. PR 1 restores a real review pool: a configurable `SURFACE_POOL_SIZE` (env, default 22) ceiling on the editorial/review selection — the limit feeding `briefing.items → persistSignalPostsForBriefing` (`src/lib/pipeline/surface-pool.ts`, wired in `data.ts` + `cron/fetch-news.ts`) — and the pipeline `surfaced` telemetry ceiling (`article-candidates.ts`) tracks it. It is a CEILING with graceful degrade (thin days yield fewer; pairs with the softened ≥5 persist floor). The public homepage display cap is unchanged: `surfacePoolSize` is passed only on the cron write path, so non-cron callers keep the public default. Observability added so the pool's composition is queryable and the silent normalize-stage drop is visible — `pipeline_article_candidates` gains `source_class` + `category` (migration `20260608120000`, additive/nullable, with a graceful-degrade insert), and `applyArticleSelectionFiltering` suppress/reject decisions now record `drop_reason='editorial_excluded'` instead of a null indistinguishable from a dedup fold. PR 2 (hard `source_class` diversity guard at selection) and PR 3 (realize `event_importance`, PRD-38) follow.
- 2026-06-08 — Selection-eligibility observability (PRD-53 remediation; selection-collapse measurement). With the review pool widened to ~22 clusters, the publishable slate still collapsed to ~2 — and the cause was not institutional concentration (the evergreen filter already strips the Fed research blogs) but Gate 1 core-eligibility in `selectPublicBriefingItems`: only `tier=core_signal_eligible` is placed; topic-cap and non-signal merely defer and backfill at `limit=22`, so they contribute zero net drops. Within core-eligibility, `ranked.score >= 58` cuts ~6 of 11 news clusters on a 57.1–57.6 hairline (substantive geopolitics/policy stories), and ~3 more clear the score but trip an unmeasured co-condition — likely `event_importance >= 52`, where `event_importance` is the PRD-38 placeholder blend. To calibrate the floor without shipping blind, `pipeline_article_candidates` now persists three per-cluster selection signals — `event_importance` (numeric), `event_type` (text), `eligibility_tier` (text) — written back from `generateDailyBriefing` by `updateArticleCandidateEligibilitySignals` (`src/lib/pipeline/article-candidates.ts`), keyed by `cluster_id`, on the cron write path only (gated by `persistPipelineCandidates`). Migration `20260608130000` is additive/nullable (`ADD COLUMN IF NOT EXISTS`), applied out-of-band per the manual-migration process; the writer degrades gracefully (the missing-column UPDATE error is swallowed) so it is order-independent with the deploy. Observability only — no eligibility threshold was changed. The staged floor change (`ranked.score` 58→~53) and the question of whether `event_importance >= 52` is a silent co-blocker that would require PRD-38 (event_importance realization) to precede it are deferred pending the per-cluster measurement these columns enable.
