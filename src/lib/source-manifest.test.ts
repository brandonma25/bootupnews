...
describe("public source manifest", () => {
  ...
  it("includes governed Batch 1, Batch 2A, and Batch 2B sources in the public.home source plan", () => {
    ...
    expect(sources.find((source) => source.id === "source-hacker-news-best")).toMatchObject({
    ...
    expect(sources.find((source) => source.id === "source-the-diplomat")).toMatchObject({
      name: "The Diplomat",
      feedUrl: "https://thediplomat.com/feed",
      topicName: "World",
      status: "active",
    });
    expect(sources.find((source) => source.id === "source-foreign-policy")).toMatchObject({
      name: "Foreign Policy",
